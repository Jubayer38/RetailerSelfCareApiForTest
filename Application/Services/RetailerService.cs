...
{
    public class RetailerService : IDisposable
    {
        private readonly RetailerRepository _repo;
        ...
        public RetailerService(string connectionString)
        {
        ...
        public int RetailerBestPractice(ComplainRequest model, int userId)
        {
            ...
            try
            {
                dynamic faqList = _repo.RetailerBestPractice(model, userId);
                return faqList;
            }
            catch (Exception ex)
            {
        ...
        public int SaveBestPracticeImage(int bestPracticeId, string base64Header, string base64Str)
        {
            try
            {
                dynamic faqList = _repo.SaveBestPracticeImage(bestPracticeId, base64Header, base64Str);
                return faqList;
            }
            catch (Exception ex)
            {
        ...
        public DataTable BestPracticesImages(GetBPImagesRequest retailer)
        {
            try
            {
                dynamic faqList = _repo.BestPracticesImages(retailer);
                return faqList;
            }
            catch (Exception ex)
            {
        ...
        {
            DataTable objList = _repo.GetRSOAndLastTime(reqModel);
            return objList;
        }
        ...
        {
            DataTable objList = _repo.SearchOffersV2(model);
            return objList;
        }
        ...
        public RSOEligibility GetITopUpStockEligibilityCheck(RetailerRequestV2 retailerRequest, out string traceMsg)
        {
            try
            {
                ...
                DataTable dt = GetRSOAndLastTime(retailerRequest);
        ...
        public DataTable GetC2STransactions(TransactionsRequest model)
        {
            try
            {
                DataTable objList = _repo.GetC2STransactions(model);
                return objList;
            }
            catch (Exception ex)
            {
        ...
        public DataTable GetC2CTransactions(TransactionsRequest model)
        {
            try
            {
                DataTable objList = _repo.GetC2CTransactions(model);
                return objList;
            }
            catch (Exception ex)
            {
        ...
        public async Task SaveFileToApiServer(List<ComplaintImageVM> imageVMs, RaiseComplSubmitRequest reqModel)
        {
        ...
        {
            string fileLocation = Path.Combine(ExternalKeys.ImagePhisycalDirPath, model.folderName);
            string path = Path.Combine(fileLocation, model.fileName);
        ...
        public string[] GetRetailersTopThreeDeno(RetailerRequestV2 retailerRequest)
        {
            try
            {
                string[] objList = _repo.GetRetailersTopThreeDeno(retailerRequest);
                return objList;
            }
            catch (Exception ex)
            {
        ...
        public DataTable RetailerDenoReport(SearchRequestV2 searchRequest)
        {
            try
            {
                DataTable objList = _repo.RetailerDenoReport(searchRequest);
                return objList;
            }
            catch (Exception ex)
            {
