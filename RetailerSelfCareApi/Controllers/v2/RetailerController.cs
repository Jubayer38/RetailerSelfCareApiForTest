...
namespace RetailerSelfCareApi.Controllers.v2
{
    ...
    public class RetailerController : ControllerBase
    {
        ...
        public async Task<IActionResult> LoginSmartPos([FromBody] LoginRequest loginRequest)
        {
            ...
            LogInViewModel logInResponse = await RetailerV2Service.LogInExternal(loginRequest, URL);
        ...
        public async Task<IActionResult> GetRetailerByRCode([FromBody] RetailerRequest retailer)
        {
            ...
            if (!string.IsNullOrWhiteSpace(traceMsg))
            {
                ...
                _logger.WriteTraceMessageInText(retailer, "v2/GetRetailer", traceMsg);
            }
        ...
        public async Task<IActionResult> RetailerBestPractice([FromBody] ComplainRequest retailer)
        {
            ...
            if (retailer.images.Any())
            {
                for (int i = 0; i < retailer.images.Count; i++)
                {
                    string image = retailer.images[i];
                    ...
                    string folderName = $"BestPractice/{retailer.retailerCode}";
                ...
                {
                    traceMsg = retailer.FileLocation = string.Join("|", imagePaths);
                }
            }
            ...
            {
                LoggerService _logger = new();
                _logger.WriteTraceMessageInText(retailer, "v2/RetailerBestPractice", traceMsg);
            }

            return new OkObjectResult(new ResponseMessage()
            {
                isError = bestPracticeId <= 0,
        ...
        {
            RetailerV2Service retailerService = new();
            DataTable bestpractice = await retailerService.BestPracticesHistory(retailer);

            if (bestpractice.Rows.Count > 0)
            {
                List<BestPracticesV2> bestpractices = bestpractice.AsEnumerable().Select(bp => HelperMethod.ModelBinding<BestPracticesV2>(bp)).ToList();
                ...
                LMSPointAdjustReq pointAdjustReq = new()
                {
                    ...
                    msisdn = retailer.iTopUpNumber.Prepend("88"),
            ...
            else
            {
                return Ok(new ResponseMessage()
                {
                    isError = false,
                    message = SharedResource.GetLocal("Success", Message.Success),
                    data = Array.Empty<BestPractices>()
                });
            }
        }
        ...
        public async Task<IActionResult> GetRetailerStock([FromBody] RetailerRequestV2 retailerRequest)
        {
            ...
            if (rsoEligibility.IsEligible == 1)
            {
                ...
                {
                    LoggerService _logger = new();
                    _logger.WriteTraceMessageInText(retailerRequest, "v2/GetRetailerStock || GetEVBalanceNdSaveToDb", traceMsg);
                }
            }
        ...
        /// </summary>
        /// <param name="ValidateIRISDenoOfferRequest">Requesting with OfferRequest's Model</param>
        /// <returns>Returing offer result</returns>
        [HttpPost]
        ...
        public async Task<IActionResult> ValidateIRISDenoOffer([FromBody] IrisOfferRequestNew offerRequest)
        {
            ...
            if (rsoEligibility.IsEligible == 1)
            {
                ...
                {
                    LoggerService _logger = new();
                    _logger.WriteTraceMessageInText(retailerRequest, "v2/ValidateIRISDenoOffer || GetEVBalanceNdSaveToDb", traceMsg);
                }
            }
            else if (rsoEligibility.IsEligible == 0)
            {
        ...
        #region Dynamic Advertisement, Disclaimer Notice, Gamification Banner
        ...
        public async Task<IActionResult> AppAdvertisement([FromBody] RetailerRequestV2 retailerRequest)
        {
            ...
            try
            {
                ...
                {
                    LoggerService _logger = new();
                    _logger.WriteTraceMessageInText(retailerRequest, "v2/AppAdvertisement", traceMsg);
                }
        ...
        public async Task<IActionResult> GamificationBanners([FromBody] RetailerRequest reqModel)
        {
            ...
            {
                LoggerService _logger = new();
                _logger.WriteTraceMessageInText(reqModel, "v2/GamificationBanners", traceMsg);
            }
        ...
        public async Task<IActionResult> SubmitGamificationResponse([FromBody] GamificationResponseReq reqModel)
        {
            ...
            {
                isError = (res <= 0),
                message = (res > 0 ? SharedResource.GetLocal("SubmitGamificationResponse", Message.SaveSuccess) : SharedResource.GetLocal("Failed", Message.Failed))
            });
        }
        ...
        [HttpPost]
        [Route(nameof(GetRecommendedComplaintCategory))]
        public async Task<IActionResult> GetRecommendedComplaintCategory([FromBody] RecommendedComplaintCategoryRequest reqModel)
        {
            try
            {
                DataTable dt = new();
                RecommendedComplaintTitle recommendedComplaintTitle = new();
                RecommendedComplaintCategoryResponse recommendedComplaintDetails = new();

                try
                {
                    //call external api for recommended complaint title
                    HttpRequestModel httpReq = new()
                    {
                        requestUrl = ExternalKeys.RaiseComplaintTitleRecommendedAiUrl,
                        requestBody = new RecommendedComplaintTitleRequest { requestId = Guid.NewGuid().ToString(), description = reqModel.complaintDetails, retailerCode = reqModel.retailerCode },
                        requestMediaType = MimeTypes.Json,
                        requestMethod = "get-complaint-category"
                    };
                    ...
                    catch (Exception ex)
                    {
                        return new OkObjectResult(new ResponseMessage()
                        {
                            isError = false,
                            message = SharedResource.GetLocal("NoDataFound", Message.NoDataFound),
                            data = recommendedComplaintDetails
                        });
                    }
                ...
                {
                    string errMsg = HelperMethod.ExMsgBuild(ex, "GetRecommendedComplaintCategory");
                    throw new Exception(errMsg);
                }
            ...
            {
                throw new Exception(HelperMethod.ExMsgBuild(ex, "GetRecommendedComplaintCategory"));
            }
        }
        ...
        public async Task<IActionResult> SubmitRaiseComplaint([FromBody] RaiseComplSubmitRequest reqModel)
        {
            ...
            if (!string.IsNullOrEmpty(reqModel.category) && reqModel.category.ToLower() == "app")
            {
                ...
                if (raiseComplInfo.Rows.Count > 0)
                {
                    ...
                    try
                    {
                        if (string.IsNullOrWhiteSpace(extSubModel.preferredLevelName) && string.IsNullOrWhiteSpace(extSubModel.preferredLevelContact))
                        {
                            ...
                            {
                                LoggerService _logger = new();
                                _logger.WriteTraceMessageInText(reqModel, "v2/SubmitRaiseComplaint || S1", errMsg);
                            }
                ...
                else
                {
                    ...
                    {
                        LoggerService _logger = new();
                        _logger.WriteTraceMessageInText(reqModel, "v2/SubmitRaiseComplaint || S2", errMsg);
                    }
            ...
            else if (!string.IsNullOrEmpty(reqModel.category) && reqModel.category.ToLower() == "superoffice")
            {
                ...
                else if (soTicketId == -1)
                {
                    ...
                    {
                        LoggerService _logger = new();
                        _logger.WriteTraceMessageInText(reqModel, "v2/SubmitRaiseComplaint || S3", errMsg);
                    }
            ...
            {
                LoggerService _logger = new();
                _logger.WriteTraceMessageInText(reqModel, "v2/SubmitRaiseComplaint", traceMsg);
            }
        ...
        public async Task<IActionResult> RaiseComplaintHistory([FromBody] HistoryPageRequestModelV2 model)
        {
            ...
            {
                LoggerService _logger = new();
                _logger.WriteTraceMessageInText(model, "v2/RaiseComplaintHistory", traceMsg);
            }
        ...
        #region Ticker Message, AppBanner, Notifications and Flash PopUps
        ...
        public async Task<IActionResult> AppBanner([FromBody] RetailerRequestV2 retailerRequest)
        {
            ...
            {
                LoggerService _logger = new();
                _logger.WriteTraceMessageInText(retailerRequest, "v2/AppBanner", traceMsg);
            }
        ...
        public async Task<IActionResult> FlashPopUp([FromBody] RetailerRequestV2 flashPopUpRequest)
        {
            ...
            try
            {
                ...
                {
                    RedisCache redis;
                    List<NotificationDetailsRedis> flashPopUpsRedis = new();
                    ...
                    {
                        LoggerService _logger = new();
                        _logger.WriteTraceMessageInText(flashPopUpRequest, "v2/FlashPopUp", traceMsg);
                    }
            ...
            if (vorInsertResult > 0)
            {
                ...
                if (model.imageList != null && model.imageList.Count > 0)
                {
                    ...
                    for (int i = 0; i < model.imageList.Count; i++)
                    {
                        ...
                        var attatchType = SaveFileHelper.GetFileExtension(imgStr);
                ...
                {
                    LoggerService _logger = new();
                    _logger.WriteTraceMessageInText(vorRequest, "v2/VORInsert", traceMsg);
                }
        ...
        public async Task<IActionResult> CreateCampaignByRetailer([FromBody] CreateCampaignByRetailerRequest model)
        {
            ...
            for (var i = 0; i < model.targets.Count; i++)
            {
                ...
                if (targetResult < 0)
                {
                    ...
                    {
                        LoggerService _logger = new();
                        _logger.WriteTraceMessageInText(model, "v2/CreateCampaignByRetailer", traceMsg);
                    }
        ...
        public async Task<IActionResult> GetQuickAccessList([FromBody] QuickAccessRequest quickAccessRequest)
        {
            ...
            RedisCache redis;
            IEnumerable<QuickAccessModel> quickAccessList = [];
            ...
            {
                LoggerService _logger = new();
                _logger.WriteTraceMessageInText(quickAccessRequest, "v2/GetQuickAccessList", traceMsg);
            }
        ...
        public async Task<IActionResult> UpdateQuickAccessListOrder([FromBody] QuickAccessUpdateRequest model)
        {
            ...
            {
                LoggerService _logger = new();
                _logger.WriteTraceMessageInText(model, "v2/UpdateQuickAccessListOrder", traceMsg);
            }
        ...
        #region Communications, and Archived
        ...
        public async Task<IActionResult> Communications([FromBody] CommunicationRequestV4 communicationRequest)
        {
            ...
            {
                throw new Exception(HelperMethod.ExMsgBuild(ex, "Communications"));
            }
        ...
        public async Task<IActionResult> GetArchivedData([FromBody] ArchivedRequest requestModel)
        {
            ...
            try
            {
                datTable = await retailerService.GetArchivedData(requestModel);
            }
            catch (Exception ex)
            {
        ...
        #region Digital Service/Product
        ...
        public async Task<IActionResult> SubmitDigitalService([FromBody] DigitalServiceSubmitRequest model)
        {
            ...
            {
                LoggerService _logger = new();
                _logger.WriteTraceMessageInText(model, "v2/SubmitDigitalService", traceMsg);
            }
        ...
        public async Task<IActionResult> GetPopUpSurvey([FromBody] RetailerRequestV2 surveyRequest)
        {
            ...
            if (isEnablePopUpSurvey)
            {
                ...
                {
                    LoggerService _logger = new();
                    _logger.WriteTraceMessageInText(surveyRequest, "v2/GetPopUpSurvey", traceMsg);
                }
        ...
        #region App Installation, AppSettings, Device Token
        ...
        public async Task<IActionResult> GetAppSettings([FromBody] AppSettingsRequest model)
        {
            ...
            else
            {
                ...
                {
                    LoggerService _logger = new();
                    _logger.WriteTraceMessageInText(model, "v2/GetAppSettings", traceMsg);
                }
        ...
        public async Task<IActionResult> GetAppSettingsV2([FromBody] AppSettingsRequest model)
        {
            ...
            else
            {
                ...
                {
                    LoggerService _logger = new();
                    _logger.WriteTraceMessageInText(model, "v2/GetAppSettingsV2", traceMsg);
                }
        ...
        public async Task<IActionResult> C2STransactions([FromBody] TransactionsRequest tranRequest)
        {
            ...
            {
                LoggerService _logger = new();
                _logger.WriteTraceMessageInText(tranRequest, "v2/C2STransactions", traceMsg);
            }
        ...
        public async Task<IActionResult> GetRSOFeedback([FromBody] RsoMemoRequest model)
        {
            ...
            try
            {
                feedbackDT = await retailerService.GetRSOFeedback(model, userId);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> GetDenoOffers([FromBody] OfferRequest deno)
        {
            ...
            List<RechargePackagesRedis> allDenoPackages = [];
            ...
            {
                LoggerService _logger = new();
                _logger.WriteTraceMessageInText(deno, "v2/GetDenoOffers", traceMsg);
            }
