...
namespace RetailerSelfCareApi.Controllers.v2
{
    ...
    public class RechargeController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetRechargeOffers([FromBody] OfferRequest offer)
        {
            RechargeV2Service rechargeService = new();
            offer.rechargeType = (int)OfferType.RechargeOffer;
            DataTable offers = await rechargeService.GetRechargeOffers(offer);
        ...
        public async Task<IActionResult> IrisRecharge([FromBody] IrisRechargeRequest rechargeRequest)
        {
            ...
            if (!string.IsNullOrWhiteSpace(traceMsg))
            {
                ...
                _logger.WriteTraceMessageInText(rechargeRequest, "v2/IrisRecharge", traceMsg);
            }
        ...
        public async Task<IActionResult> EvRecharge([FromBody] EVRechargeRequest rechargeRequest)
        {
            ...
            {
                LoggerService _logger = new();
                _logger.WriteTraceMessageInText(rechargeRequest, "v2/EvRecharge", traceMsg);
            }
        ...
        public async Task<IActionResult> MultiEvRecharge([FromBody] EVRechargeRequest rechargeRequest)
        {
            ...
            bool success = rechargeInfo.Where(f => f.isSuccess == true).Any();
            ...
            {
                LoggerService _logger = new();
                _logger.WriteTraceMessageInText(rechargeRequest, "v2/MultiEvRecharge", traceMsg);
            }
        ...
        /// <summary>
        /// API for getting IRIS offers with/without amount. Also include admin created offers when amount available.
        /// This api is applicable from app version v6.4.0
        /// </summary>
        /// <param name="offerRequest"></param>
        /// <returns></returns>
        [HttpPost]
        [Route(nameof(ShowAllIRISOffersV2))]
        public async Task<IActionResult> ShowAllIRISOffersV2([FromBody] IrisOfferRequestNew offerRequest)
        {
            string userAgent = HttpContext.Request?.Headers.UserAgent.ToString();
            offerRequest.userAgent = userAgent;
            RechargeV2Service rechargeService = new();
